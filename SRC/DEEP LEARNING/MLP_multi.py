...
train_labels_folder = "../../DATA MODIFIED/TrueAngles_TrainMULTI/"
test_labels_folder = "../../DATA MODIFIED/TrueAngles_TestMULTI/"
...
#train_labels_folder = "/Users/admin-21760/Desktop/M2/Computational Systems and Structural Biology/Bioinformatics of RNA and non-coding world/RNA_angles/RNA-project-angles/data/TrueAngles_TrainMULTI/"
#test_labels_folder = "/Users/admin-21760/Desktop/M2/Computational Systems and Structural Biology/Bioinformatics of RNA and non-coding world/RNA_angles/RNA-project-angles/data/TrueAngles_TestMULTI/"
...
for train_file, test_file in zip(train_files, test_files):
    ...
    mlp_multi = MLPClassifier(hidden_layer_sizes=(100,), max_iter=50, random_state=42)
    ...
    for epoch in range(500):
        mlp_multi.partial_fit(train_x, train_y, classes=np.unique(train_y))
        train_loss = mlp_multi.loss_curve_

        train_accuracy = accuracy_score(train_y, mlp_multi.predict(train_x))
        train_accuracy_epoch.append(train_accuracy)

        val_accuracy = accuracy_score(val_y, mlp_multi.predict(val_x))
        val_accuracy_epoch.append(val_accuracy)
    
        test_accuracy = accuracy_score(test_y, mlp_multi.predict(test_x))
        test_accuracy_epoch.append(test_accuracy)
        test_loss = mlp_multi.loss_
...
for test_file in test_files:
    ...
    test_x = test_x.reshape(test_x.shape[0], -1)
    test_x = scaler.transform(test_x)
    test_predictions = mlp_multi.predict(test_x)

    output_file = os.path.join("../../RESULTS/Y_pred_multi/", f"{protein_name}_predictions.txt")
    
    #output_file = os.path.join("/Users/admin-21760/Desktop/M2/Computational Systems and Structural Biology/Bioinformatics of RNA and non-coding world/RNA_angles/RNA-project-angles/data/Y_pred_multi/", f"{protein_name}_predictions.txt")
